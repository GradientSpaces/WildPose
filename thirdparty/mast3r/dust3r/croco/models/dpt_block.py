...
class FeatureFusionBlock_SameShape(nn.Module):
    """Compared to the standard one (FeatureFusionBlock_custom). This model fuse features
    with the same shape and output a fused feature with the same shape
    """
    ...
    def __init__(
        ...
        Args:
            features (int): number of features
        """
        super(FeatureFusionBlock_SameShape, self).__init__()
...
class DPTOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        if self.head_type == 'regression':
        ...
        elif self.head_type == 'regression_motion_mask':
            # The head used for motion mask regression in WildPose.
            
            # Downsampling blocks to bring H/4 and H/2 layers to H/8
            self.downsample_l0 = nn.Sequential(
                nn.Conv2d(feature_dim, feature_dim, kernel_size=3, stride=2, padding=1),
                nn.ReLU(True)
            )
            
            # Simpler Layer Fusion Heads (for downsampled high-resolution ones)
            self.head_fusion1 = FeatureFusionBlock_SameShape(feature_dim)
            self.head_fusion2 = FeatureFusionBlock_SameShape(feature_dim)
            
            # Final Head
            self.head = nn.Sequential(
                nn.Conv2d(feature_dim, feature_dim // 2, kernel_size=3, stride=1, padding=1),
                nn.ReLU(True),
                nn.Conv2d(feature_dim // 2, self.num_channels, kernel_size=1, stride=1, padding=0),
            )
            ...
            raise ValueError('DPT head_type must be "regression", "semseg", or "regression_motion_mask".')
