...
class SparseBlock {
  public:
    ...
    torch::Tensor solve(const double lm=0.0001, const double ep=0.1) {

      torch::Tensor dx;

      // (1) symmetrize numerically (cheap + helps LLT)
      Eigen::SparseMatrix<double> AT = A.transpose();
      Eigen::SparseMatrix<double> Asym = 0.5 * (A + AT);

      // Eigen::SparseMatrix<double> L(A);
      Eigen::SparseMatrix<double> L(Asym);
      ...
      if (solver.info() == Eigen::Success) {
        ...
        // final failure log (printed once per failed system)
        const double mean_diag = Asym.diagonal().mean();
        const double min_diag  = Asym.diagonal().cwiseAbs().minCoeff();
        std::cout << "SparseBlock::solve: Cholesky failed; final lm=" << lm << ", ep=" << ep
                  << ", N=" << N << ", M=" << M
                  << ", mean(diag(A))=" << mean_diag
                  << ", min(|diag(A)|)=" << min_diag
                  << "\n" << std::endl;
...
std::vector<torch::Tensor> ba_cuda(
    ...
    torch::Tensor metric_d_mask_map)
{
  ...
  for (int itr=0; itr<iterations; itr++) {
    ...
    if (motion_only) {
      ...
      auto metric_d_mask = metric_d_mask_map
                     .index({kx, "..."})
                     .view({-1, ht*wd});

      torch::Tensor m = (disps_sens.index({kx, "..."}) > 0).to(torch::TensorOptions().dtype(torch::kFloat32)).view({-1, ht*wd});
      // torch::Tensor C = accum_cuda(Cii, ii, kx) + m*metric_d_mask * alpha + (1 - m*metric_d_mask) * eta.view({-1, ht*wd});
      // torch::Tensor w = accum_cuda(wi, ii, kx) - m*metric_d_mask * alpha * (disps.index({kx, "..."}) - disps_sens.index({kx, "..."})).view({-1, ht*wd});
