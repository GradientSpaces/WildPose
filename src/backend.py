...
class Backend:
    ...
    @torch.no_grad()
    def backend_ba(self, t_start, t_end, steps, graph, nms, radius, thresh, max_factors, t_start_loop=None, loop=False):
        """ main update """
        ...
        edges = (graph.ii.clone().cpu().numpy(), graph.jj.clone().cpu().numpy())

        graph.clear_edges()
        return edge_num, edges[0], edges[1]

    @torch.no_grad()
    def dense_ba(self, steps=6, release_depth_reg=True):
        t_start = 0
        ...
        if metric_depth_reg_activated and release_depth_reg:
            self.video.metric_depth_reg = False

        graph = FactorGraph(self.video, self.update_op, self.cfg,
                            motion_mask_detector=self.motion_mask_detector,
                            feature_extractor=self.feature_extractor,
                            device=self.device, corr_impl='alt', max_factors=max_factors)
        n_edges, edge_i, edge_j = self.backend_ba(t_start, t_end, steps, graph, nms, radius,
                          thresh, max_factors)
        ...
        self.video.metric_depth_reg = metric_depth_reg_activated
        return n, n_edges, edge_i, edge_j
    ...
    def loop_ba(self, t_start, t_end, steps=6, motion_only=False, local_graph=None):
        ...
        n_edges, _, _ = self.backend_ba(t_start, t_end, steps, graph, nms, radius, thresh, 
