    ...
    def run(self, stream:BaseDataset):
        ...
        for i in range(len(stream)):
            ...
            with torch.no_grad():
                ...
                force_to_add_keyframe = self.motion_filter.track(timestamp, image, intrinsic,is_last= (i==len(stream)-1))

                # local bundle adjustment
                self.frontend(force_to_add_keyframe,is_last= (i==len(stream)-1))
            curr_kf_idx = self.video.counter.value - 1
            
            if curr_kf_idx != prev_kf_idx and self.video.counter.value > self.frontend.warmup:
                if self.enable_online_ba and curr_kf_idx >= prev_ba_idx + self.ba_freq:
                    ...
                    self.online_ba.dense_ba(2, release_depth_reg=False)
