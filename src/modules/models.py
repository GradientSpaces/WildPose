...
from src.modules import ConvGRU, GradientClip,Mast3rEncoder
...
class WildPoseNet(nn.Module):
    def __init__(self, motion_head_type=None):
        super(WildPoseNet, self).__init__()
        self.fnet = Mast3rEncoder(output_dim=128, norm_fn='instance')
        self.cnet = Mast3rEncoder(output_dim=256, norm_fn='none')
        self.update = UpdateModule()
        
        self.feature_extractor = load_mast3r()
        self.feature_extractor.eval()
        
        if motion_head_type is not None:
            l2 = self.feature_extractor.dec_depth
            ed = self.feature_extractor.enc_embed_dim
            dd = self.feature_extractor.dec_embed_dim
            self.motion_mask_detector = PixelwiseTaskWithDPT(num_channels=1,
                                                            feature_dim=256,
                                                            last_dim=128,
                                                            hooks_idx=[0, l2*2//4, l2*3//4, l2],
                                                            dim_tokens=[ed, dd, dd, dd],
                                                            postprocess=None,
                                                            head_type=motion_head_type)