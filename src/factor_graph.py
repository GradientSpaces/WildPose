...
from src.modules import CorrBlock, AltCorrBlock, mast3r_extractor_feat
...
class FactorGraph:
    ...
    def __init__(self, video, update_op, cfg,
                 motion_mask_detector=None, feature_extractor=None,
                 device="cuda:0", corr_impl="volume", max_factors=-1):
        self.video = video
        self.update_op = update_op
        self.cfg = cfg
        self.motion_mask_detector = motion_mask_detector
        ...
        self.corr, self.net, self.inp, self.dec1 = None, None, None, None
    ...
    def rm_keyframe(self, ix):
        ...
        with self.video.get_lock():
            ...
            # Remove any decoding cache entries involving the removed frame
    ...
    @torch.no_grad()
    def extract_motion_mask(self, ii, jj, batch_size=20):
        # print("*"*200)
        # start_time = time()
        """ run feeature extraction networks """
        torch.cuda.empty_cache()

        # Identify which pairs need to be computed
        to_compute_ii, to_compute_jj = [], []

        for k, (i, j) in enumerate(zip(ii, jj)):
            key = (i.item(), j.item())
            if key not in self.video.mask_cache:
                to_compute_ii.append(i)
                to_compute_jj.append(j)

        if to_compute_ii:
            to_compute_ii = torch.stack(to_compute_ii)
            to_compute_jj = torch.stack(to_compute_jj)
            all_indices = torch.cat([to_compute_ii, to_compute_jj])
            
            # all_indices = torch.cat([ii, jj])
            unique_indices, inverse_indices = torch.unique(all_indices, return_inverse=True)
            new_ii = inverse_indices[:len(to_compute_ii)].to(self.device)
            new_jj = inverse_indices[len(to_compute_ii):].to(self.device)
            images = self.video.images[unique_indices.cpu()].to(self.device)

            with torch.cuda.amp.autocast(False):
                # images = images / 255.0  #[B, 3, H, W]
                # Mean and std is 0.5 (from mast3r)
                mean = torch.as_tensor([0.5, 0.5, 0.5], device=images.device)
                std = torch.as_tensor([0.5, 0.5, 0.5], device=images.device)
                images = images.sub_(mean[:, None, None]).div_(std[:, None, None])

            _,_,H,W = images.shape
            img_shape = torch.tensor((H,W), device=images.device)
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast(False):
                    feat, pos = mast3r_extractor_feat(self.feature_extractor, images, img_shape)
                    _, hdd_wdd, D = feat.shape
                    assert hdd_wdd == int(H*W/256), "Feature extractor output shape mismatch"
                    # Variable used for motion mask detection
                    feat1, pos1 = feat[new_ii], pos[new_ii]
                    feat2, pos2 = feat[new_jj], pos[new_jj]
                    
                    # Batched Decoding
                    for k in range(0, S, batch_size):
                        dec1_batch, _ = self.feature_extractor._decoder(
                            feat1[k:k+batch_size], pos1[k:k+batch_size],
                            feat2[k:k+batch_size], pos2[k:k+batch_size]
                        )

                        motion_mask_logit = self.motion_mask_detector(
                            dec1_batch,
                            img_shape
                        ).reshape(1,-1,H//8,W//8)

                        with self.video.get_lock():
                            for kk in range(new_ii[k:k+batch_size].shape[0]):
                                video_key = (to_compute_ii[k+kk].item(), to_compute_jj[k+kk].item())
                                self.video.mask_cache[video_key] = torch.sigmoid(motion_mask_logit[0,kk]).cpu()

        motion_mask = torch.zeros((1, len(ii), self.ht, self.wd), device=self.device, dtype=torch.float)
        for k, (i, j) in enumerate(zip(ii, jj)):
            key = (i.item(), j.item())
            motion_mask[0,k] = self.video.mask_cache[key].to(self.device)
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False,
        ...
        if self.cfg['tracking']['mask_detector']:
            assert self.motion_mask_detector is not None, "Motion mask detector is none"
            motion_mask = self.extract_motion_mask(self.ii, self.jj)
            weight = weight * motion_mask.unsqueeze(-1)
        ...
        for step in range(steps):
            ...
            for i in range(0, self.jj.max()+1, s):
                ...
                with torch.amp.autocast('cuda', enabled=True):
                    ...
                    if self.cfg['tracking']['mask_detector']:
                        assert self.motion_mask_detector is not None, "Motion mask detector is none"
                        motion_mask = self.extract_motion_mask(iis, jjs)
                        weight = weight * motion_mask.unsqueeze(-1)
